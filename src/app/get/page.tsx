...
export default function GetResultsPage() {
  ...
  const runAllTests = async () => {
    ...
    // Critical APK Handshake Endpoints
    const endpoints = [
      ...
      { method: 'GET', path: '/api/xxapi/linkKyc?mobileNo=919060873927' }, // Live KYC Check
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-code selection:bg-blue-600 selection:text-white">
      ...
      <header className="border-b border-slate-200 bg-white/95 backdrop-blur-xl sticky top-0 z-50 px-8 h-20 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="font-headline font-black text-xl tracking-tight">API AUDIT CENTER</h1>
        ...
        <div className="flex items-center gap-4">
          ...
          <Button 
            ...
            className="bg-blue-600 hover:bg-blue-700 text-white font-black text-[12px] uppercase px-8 h-12 shadow-xl shadow-blue-600/20 rounded-2xl transition-all hover:scale-[1.02] active:scale-[0.98] flex gap-3"
          >
      ...
      <main className="max-w-7xl mx-auto p-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <Card className="border-slate-200 shadow-sm bg-white hover:border-blue-500/30 transition-all">
          ...
          <Card className="border-slate-200 shadow-sm bg-white hover:border-emerald-500/30 transition-all">
            <CardContent className="p-6">
                    ...
                    <tr key={log.id} onClick={() => setSelectedLog(log)} className="hover:bg-blue-50/40 transition-all cursor-pointer group">
                      ...
                      <td className="p-6 text-xs font-bold text-slate-700">
                        <div className="flex flex-col">
                          ...
                          {log.endpoint.includes('linkKyc') && <span className="text-[9px] text-blue-500 uppercase font-black mt-1">Old Server Proxy</span>}
                        </div>
                      </td>
                      <td className="p-6">
                      ...
                      <td className="p-6 text-right">
                        <Button variant="ghost" size="sm" className="rounded-xl hover:bg-blue-100 text-blue-600 group-hover:scale-110 transition-all">
        ...
        <DialogContent className="max-w-5xl bg-white border-slate-200 text-slate-900 rounded-[3rem] shadow-2xl p-0 overflow-hidden">
          <div className="p-12 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
            <div>
              ...
              <DialogDescription className="text-slate-400 font-bold text-[12px] uppercase tracking-widest mt-4 flex items-center gap-3">
            ...
            <Button 
              ...
              className="h-16 px-12 gap-4 bg-blue-600 hover:bg-blue-700 text-white font-black text-[13px] uppercase rounded-[1.5rem] shadow-2xl shadow-blue-600/20 transition-all hover:scale-[1.02] active:scale-[0.98]"
            >
          ...
          <div className="p-12 bg-white space-y-8">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 shadow-sm">
                <p className="text-[10px] font-black uppercase text-slate-400 mb-2">Endpoint</p>
                <p className="text-sm font-bold text-slate-800 truncate">{selectedLog?.endpoint}</p>
              </div>
              <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 shadow-sm">
                <p className="text-[10px] font-black uppercase text-slate-400 mb-2">Status Code</p>
                <p className={`text-sm font-black uppercase ${selectedLog?.status === 'success' ? 'text-emerald-600' : 'text-rose-600'}`}>
                  {selectedLog?.statusCode} - Secure
                </p>
              </div>
              <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 shadow-sm">
                <p className="text-[10px] font-black uppercase text-slate-400 mb-2">Protocol</p>
                <p className="text-sm font-bold text-slate-800">HTTPS/TLS1.3</p>
              </div>
              <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 shadow-sm">
                <p className="text-[10px] font-black uppercase text-slate-400 mb-2">Audit Time</p>
                <p className="text-sm font-bold text-slate-800">{new Date(selectedLog?.timestamp || '').toLocaleTimeString()}</p>
              </div>
            </div>
